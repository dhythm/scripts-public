...
def get_bounding_box(bounding_poly: documentai.BoundingPoly) -> Tuple[float, float, float, float]:
...
def batch_process_document_with_bounds(
    ...
    """
    # 1. GCSクライアントとDocument AIクライアントを初期化
    ...
    # 2. GCSバケットの準備
    # GCSのロケーションはDocAIと完全一致しない場合があるため、'us'などを指定
    ...
    # 5. 非同期処理リクエストの作成と実行
    gcs_input_config = documentai.GcsInputConfig(gcs_uri=gcs_input_uri, mime_type=mime_type)
    ...
    request = documentai.BatchProcessRequest(
        name=name,
        input_documents=documentai.BatchDocumentsInputConfig(gcs_input_config=gcs_input_config),
    ...
    # タイムアウトを設定して完了を待つ (例: 30分)
    try:
        operation.result(timeout=1800)
        print("非同期処理が完了しました。")
    except Exception as e:
        raise Exception(f"非同期処理がタイムアウトまたは失敗しました: {e}")

    # 6. GCSから結果をダウンロードして解析
    print("GCSから処理結果をダウンロード中...")
    extracted_text = []
    
    output_blobs = list(bucket.list_blobs(prefix=gcs_output_prefix))
    if not output_blobs:
        raise Exception("処理結果が見つかりませんでした。Document AIの処理中にエラーが発生した可能性があります。")
